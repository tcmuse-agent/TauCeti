...
* `hasFDerivAt_extChartAt_mulInvariantExp_smul_mul_mulInvariantExp_smul_zero`: two exponential
...
theorem hasFDerivAt_extChartAt_mulInvariantExp_smul_mul_mulInvariantExp_smul_zero
    [CompleteSpace E]
    [LieGroup I (minSmoothness ℝ 3) G] [IsManifold I 1 G]
    (X Y : GroupLieAlgebra I G) :
    let _ : ContMDiffMul I 1 G :=
      ContMDiffMul.of_le (m := 1) (n := minSmoothness ℝ 3) (by simp)
    let _ : BoundarylessManifold I G := ContMDiffMul.boundarylessManifold
    let _ : T2Space G := t2Space_of_lieGroup (I := I) (n := minSmoothness ℝ 3)
    HasFDerivAt
      (fun t : ℝ => extChartAt I (1 : G)
  ...
  have hmul := (contMDiff_mul (G := G) I (minSmoothness ℝ 3)).contMDiffAt
